...
A CLI command is a sequence of printable ASCII characters terminated with \texttt{\textbackslash{}r\textbackslash{}n} (ASCII codes 13, 10); the sequence must not be also a valid SLCAN command. Every CLI command returns a response that begins with the exact copy of the received command, terminated with \texttt{\textbackslash{}r\textbackslash{}n}, then followed by an arbitrary number of lines, each terminated with \texttt{\textbackslash{}r\textbackslash{}n}, and finalized with the ASCII END-OF-TEXT character (code 3) immediately followed by the final \texttt{\textbackslash{}r\textbackslash{}n}.

For example, a command \texttt{cfg   list} (the excessive spaces were added for demonstrational purposes) may produce the following response (non-printable characters are shown with escape sequences, e.g. \texttt{\textbackslash{}r}):
