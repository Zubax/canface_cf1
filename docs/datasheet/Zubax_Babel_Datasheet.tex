...
\texttt{t} & \texttt{iiid*} & Same, 11-bit data frame \\
\texttt{R} & \texttt{iiiiiiiid} & Same, 29-bit RTR frame \\
\texttt{r} & \texttt{iiid} & Same, 11-bit RTR frame \\
\end{ZubaxSimpleTable}
...
\begin{itemize}
\item Loopback frames will be appended the character \texttt{L} at the end of the block. This option allows the host to distinguish between received frames and the frames that were received from the bus.
\end{itemize}

For example, a frame \texttt{T12345678401234568} will be returned back as follows, depending on the configuration:

\begin{ZubaxSimpleTable}{SLCAN notification example}{|l l X|}
Flags enabled & Timestamping enabled	& Representation \\
No & No & \texttt{T12345678401234568$\backslash$r} (no changes) \\
No & Yes & \texttt{T123456784012345680BED$\backslash$r} (transmission timestamp 3053 milliseconds) \\
Yes & No & \texttt{T12345678401234568L$\backslash$r} (added flag \texttt{L})\\
Yes & Yes & \texttt{T123456784012345680BEDL$\backslash$r} (both of the above) \\
\end{ZubaxSimpleTable}
...
A CLI command is a sequence of printable ASCII characters terminated with \texttt{$\backslash$r$\backslash$n} (ASCII codes 13, 10); the sequence must not be also a valid SLCAN command. Every CLI command returns a response that begins with the exact copy of the received command, terminated with \texttt{$\backslash$r$\backslash$n}, then followed by an arbitrary number of lines, each terminated with \texttt{$\backslash$r$\backslash$n}, and finalized with the ASCII END-OF-TEXT character (code 3) immediately followed by the final \texttt{$\backslash$r$\backslash$n}.

For example, a command \texttt{cfg   list} (the excessive spaces were added for demonstrational purposes) may produce the following response (non-printable characters are shown with escape sequences, e.g. \texttt{$\backslash$r}):
...
\begin{itemize}
\item \texttt{cfg list} - list all configuration parameters, one per line, each line is formatted as follows:\\ \texttt{name = value [min, max] (default)}. Floating point parameters are rendered with explicit decimal separator, e.g.  \texttt{1.0} rather than \texttt{1}.
\item \texttt{cfg set <name> <value>} - assign the parameter a new value. The response may contain \texttt{Error:} followed by the description of the error in case of failure. It is recommended to execute \texttt{cfg list} afterwards to check if the value was updated.
\item \texttt{cfg save} - save the current configuration into the non-volatile memory. With firmware v1.1 this command became redundant and it should no longer be invoked manually,  because configuration parameters are saved into the non-volatile memory automatically upon modification.
\item \texttt{cfg erase} - remove the stored configuration from the non-volatile memory. Next restart will reset all parameters to defaults.
\end{itemize}
